# exam-kb-agent · 考试知识库 Agent

> 把**任何考试**的资料与错题整理成结构化、可检索、可复习的知识库。
> **纯本地 · 零服务器 · BYOK（用你自己的 API Key）· Markdown 为事实源。**
>
> **Windows 桌面版 V1**，架构预留 Android / iPadOS / macOS / iOS 扩展位。

## 支持范围（现状与边界，如实写）

| 科目 | 状态 | 本体规模 |
|---|---|---|
| **考研数学一 / 二 / 三** | 🟢 **完整支持**（本体 + 考频 + 组卷模板 + 评测集） | 19/11/42 个考频单元 · **270 个叶子知识点** |
| 考研英语 / 政治 / 专业课 | ⚪ **未开始** | 见下方 [路线图](#路线图为什么现在只有数学) |
| 其他考试（四六级 / 法考 / 公考…） | ⚪ 未开始 | 同上 |

> ⚠️ **诚实声明**：仓库名叫 `exam-kb-agent`，是因为**骨架与科目无关**；
> 但**今天真正能用的只有考研数学**。别的科目缺的不是代码，是**本体资产**（那把尺子）。
> 我们不想用"支持任意考试"这句话把没做的事说成做完了。

---

## 这个项目要解决什么

市面上的错题 App 都在做「整理得漂亮」，但考研党真正的问题是**这些题以后还会错**。

本项目的立足点：

| 竞品的做法 | 本项目的做法 |
|---|---|
| 拍照 → 给答案 | 拍照 → **记录你为什么错** → 按错因开处方 |
| AI 的标注从哪来 | **受控知识点词表**，AI 只能从 270 个叶子知识点里选，且必须落在候选集内 |
| 按知识点统计错题数 | **六维错因 × 考频权重**，区分"不会"和"算错" |
| 组卷靠随机抽题 | **真题结构 + 薄弱点加权 + 考频软偏好** |
| 数据锁在服务器 | **Markdown 存盘**，用户能用 Git 管理、能迁移到 Obsidian |
| 每次打标都花钱 | **按题干指纹 + 模型缓存**，并在本机算出"我花了多少" |

**核心闭环**：录入一道错题（≤60 秒）→ AI 打标 → 进错题本 → FSRS 按遗忘曲线提醒复习 → 攒够了出一份针对性检测卷。

**V1 只需要证明一件事**：用户录完第一道题之后，**会想录第二道题**。

> 已用真实资料验证过这条闭环：660 线代（数学二）题目段 71 页 → **201 道题入库，0 失败**，
> 全程费用 ¥0（智谱免费视觉模型）。过程中"防模型编造答案"的闸门真的拦下了 3 页模型自己解出来的答案。

---

## 当前状态

> 详细进度见 **[`docs/PROGRESS.md`](docs/PROGRESS.md)**

| 里程碑 | 状态 |
|---|---|
| M0 地基（知识资产 + 格式规范） | 🟢 完成 |
| M1 项目骨架（抽象层 + 断点 + 主题） | 🟢 完成 |
| M2 数据层（Drift + Markdown + 索引） | 🟢 完成 |
| M3 标注引擎（LLM + 召回 + 评测 + 缓存/用量） | 🟢 完成（报数集 Top-1 **83.3%**，目标 ≥80% 达标） |
| M4 录入闭环（公式键盘 + 编辑页 + 错因 + 保存） | 🟢 主流程完成（图片识别按计划不做） |
| M5 错题本 + 复习（列表/检索/详情/删除 + FSRS 会话） | 🟢 主流程完成 |
| M6 组卷 + 导出（贪心组卷 + 三版式 PDF + Markdown 包） | 🟢 主流程完成 |
| M7 批量导入（视觉解析 + 逐题核对 + 批量打标） | 🟢 主流程完成（真机验收待做，见 T44） |
| M8 画像 + 打磨 | 🟢 代码完成（只剩"连续自用 7 天"这条真机验收） |

**V1 必做十项（F1–F10）到这里全部有了界面**，八个导航目的地全部指向真实页面。

```
flutter test    → 739 个用例全绿
flutter analyze → No issues found
```

### 界面一览

八个导航目的地（Ctrl+1…8）全部指向真实页面：

| 页面 | 做什么 |
|---|---|
| 知识库 | 三级知识点树 + **图谱**（滚轮缩放/拖动平移/点节点看详情）与**大纲**两种视图 |
| 错题本 | 三种排序 + FTS5 中文检索 + 详情抽屉 + 编辑/删除 |
| 录入 | 公式快捷键盘 + 考点选择器（按名称**与别名**搜索）+ 宽屏实时预览 |
| 复习 | 揭晓式卡片 + 三档打分 + 键盘 1/2/3 + 未来 7 天分布 |
| 批量导入 | 选文件夹/多图/多 PDF → 预估花费 → 视觉模型解析 → 逐题核对 → 入库 → 打标 |
| 组卷 | 模板 → 偏好 → 组卷 → 预览 → 保存/导出三版式 PDF |
| 画像 | 最薄弱考点 + 各章节掌握 + 错因分布 + 30 天复习曲线 |
| 设置 | AI 用量台账 / 缓存 / 数据导出 / 每日提醒 |

> 📷 **截图待补**（需要在真机上开一次 GUI 并截图，这件事目前由作者手工做）。
> `ui-mockups/` 里是**立项时的原型**，不是当前界面 —— 别把它当截图看。

### 掌握度画像（F7 已完成）

「画像」页（Ctrl+7）：总览 → 最薄弱的考点 → 各章节掌握 → 错因分布 → 30 天复习曲线。

**掌握度是"此刻"重算的，不是复习那一刻的记录。** `user_problem_state.mastery`
存的是打分那一刻的可提取性，而它会随时间衰减 —— 列里的值不会跟着变（T37）。
用那个快照做画像不是"略有偏差"，是**系统性失真**：拖得越久没复习的考点越被
高估，而它恰好是最该补的。现在按 `now` 用 FSRS 的可提取性现算，
错题本列表里的"掌握 X%"也走同一个函数 —— 否则同一个考点会出现
"列表说 90%、画像说 40%"，用户无法判断该信哪个。

**"最薄弱"是启发式，所以排序理由摆在每一行上：**

```
weakness = (1 − mastery) × (1 + log2(1 + wrongCount))
```

掌握度低是主因；错得多说明它反复暴露问题，取对数是为了不让长尾
（某人某题错 20 次）压倒"掌握度更低"的考点。验收标准写的是"排序符合主观感受"，
而主观感受只能由用户自己判断 —— 所以每行都给出掌握度与错次数，
页脚写明这是启发式。排序不合心意时，用户能看出是哪个数导致的。

**有多少数据说多少话**：每个考点都显示 `2/2 题有记录` 这样的样本量。
一个由 1 道题算出的"掌握 30%"和一个由 12 道题算出的可信度完全不同；
只给百分比就是在暗示一个它没有的精度。没有复习数据的条不画 0% 的进度条
（那看起来像"完全不会"，而真相是"还不知道"），用空槽表示空缺。

**错因分布做了一次 schema 迁移（v3 → v4）**：给 `problems_index` 加了
`error_causes` 列。指数表本来就是为"不必回头读 Markdown"存在的，
而错因分布是聚合表 —— 不冗余就得读 5000 个文件。但旧行的这一列会一直是空的，
所以画像明确提示"有 N 道题没有错因数据，重建一次索引就会补全"：
一份**只统计了迁移之后新录的题**的分布看起来有数据、实际是错的。

**复习曲线**：最近 30 天按天聚合，**没复习的日子留在序列里** ——
去掉它们会把"停了半个月"画成一条连续的线，而那正是最该看见的信息。
"过关率"用计数算（评分 ≥2 的次数 / 总次数），不是从平均评分反推的：
平均 2.5 既可能是一半忘了+一半轻松，也可能全部吃力，反推出来的比率是编的。

### 打磨（M8 的一半）：先量再改

V1 验收表里有一条能自动化的指标：「错题本列表 5000 题滚动不掉帧」。
灌 5000 题实测（`flutter test test/list_perf_test.dart`，跑满要设 `DSH_PERF=1`）：

| 指标 | 实测 | 处理 |
|---|---|---|
| 索引查询：全表 vs 只取列表用得到的 8 列 | 50 ms → 26 ms | **省 47%**，已改 |
| 列表数据构建（读 + 排序） | 146–249 ms | 打开页签的一次性开销，可接受 |
| 滚动时**同时存活**的列表项 | **12–13 / 5000** | 懒加载本来就是对的，不动 |

两条要点：`problems_index` 里 `search_tokens`（逐字加空格的全文）与
`parse_warnings` 两列**列表一个字符都不用**，早先却是整表读出来的；
而"滚动会不会建 5000 个 widget"这个担心**实测不存在** ——
`ListView.separated` 本来就是懒的。这条还加了断言（存活项 < 100），
哪天有人换成 `ListView(children: [...])` 会立刻红。

> ⚠️ 那个文件**刻意不对帧时间下断言**：widget 测试的 `pump` 含框架自身开销，
> 数量级高于真机，拿它当帧预算只会得到一条随机失败的用例。
> 真机帧率仍要用 DevTools 看（待办 N15）。

同时修掉三处「把**读不出来**渲染成**看起来正常的空态**」：
AI 用量面板读库失败时说「还没有调用过 AI」、缓存条数说「0 道」、
组卷模板 JSON 损坏时说「这个科目还没有可用模板」。
三处都会让用户以为"功能就是这样"或"我没花钱"，既不会报障也查不到原因。

空态/错误态统一到 `core/widgets/state_views.dart`。真正的毛病不是"长得不一样"，
而是**有的页面出错了没有出路** —— 错题本和录入页原先只有一行灰字，
用户唯一的办法是重启应用。现在错误态一律带**可选中复制**的原始错误与重试按钮。

### 批量导入（M7 已完成）

「批量导入」页（Ctrl+5）：选文件夹 / 多张图片 / 多个 PDF → 预估花费 →
云端视觉模型解析 → 逐题核对 → 批量入库 → 批量打标。

三个决定值得单独说：

**不引入 pdfium。** Claude 与 Gemini 能**直接读 PDF**，而批量导入本来
就要调云端视觉模型 —— 为同一件事既付 token 又往包里塞 10–15 MB 的渲染
引擎没有道理。代价是整份 PDF 作为一次请求发出，页数多了会被输出长度截断，
所以上限与"整份发送"这件事在导入前就明确告诉用户。

**能力判定提前做，而且是三态。** 「确定不支持 / 已知支持 / 无法确认」。
DeepSeek 是纯文本模型，选它点导入会被**拦住**并告知换哪家 ——
否则就是 100 次注定失败的调用，用户等几分钟还白花钱。
自建网关不认识的模型判为"无法确认"：放行但提示，不把用户挡在门外。

**防模型编造答案。** 模型很擅长解题，给它一张只有题干的照片，它会顺手
把答案算出来填上。提示词要求它用 `answer_from_source` 声明答案是不是
抄自原文，而代码**按声明办事**：说"不是抄的"就丢弃那份答案。
用户拿到一份编造答案的错题本，比拿到没答案的错题本糟得多。

### 智能组卷（F8 已完成）

「组卷」页（Ctrl+6）：选模板 → 调偏好 → 组卷 → 预览 → 保存/导出。
数学三科 × 3 模板（真题全卷 22 题 / 突击模考 11 题 / 错题专练 15 题）。

**难度按题号给，不按全局比例给** —— 比如解答题是 `[2,2,2,3,3,3]`。
换成"整卷 30/50/20"能凑出同样比例，但难度分布是乱的，而真题手感恰恰
在于由易到难递进。

**不静默降级**是这一层的核心纪律：填不上的题位、放宽的难度、估算的分值
全部显示在预览上方并附在导出 PDF 末尾 —— 用户能看到"这份卷子哪里不像真题"，
而不用自己数。

三版式 PDF：**试卷**（不含答案、留演算空间）/ **解析卷**（题干+答案+解析）/
**错题本**（按考点分组）。公式用 Flutter 自己的渲染器**光栅化成位图**嵌进
PDF —— 这样 PDF 与屏幕逐像素一致，不必维护第二套排版实现。

**中文也能正常显示（T43 已修）**：`pdf` 包内置字体不含中文字形，早先导出的
卷子上中文是一片空白。修法是取**本机已装**的中文字体，挂在文档主题的
`fontFallback` 上 —— 拉丁字母仍然走 Helvetica（PDF 标准字体，不嵌入、
字形也更好看），只有汉字走中文字体。

这里有个当初想岔了的地方：一开始把选择框成"联网下载字体"或"随包内置
5–10 MB 字体"，两条都不划算。看清 `pdf` 包的行为之后发现那是个**假二选一**：

- 它写文件时**只嵌入用到的字形**（自带 `Font SubSetting CN` 测试），
  所以 PDF 本身不会变大 —— 实测一份两题的卷子 **18–28 KB**，
  而源字体是 9.29 MB；
- 中文 Windows 上这些字体**本来就在** `%SystemRoot%\Fonts`。读用户机器上
  已装的字体、把用到的字形嵌进用户自己的文档，是任何 Windows 程序打印时
  都在做的事，**不涉及再分发字体文件**。

于是既不用联网、也没给安装包加一个字节。唯一的实质让步：`pdf` 包不支持
TTC 字体集合，所以**宋体**（`simsun.ttc`）用不了，退到**黑体**
（`simhei.ttf`，单一 TTF）。万一机器上一个中文字体都没有（英文版 Windows），
导出后会如实提示并给出出路 —— 安静地输出一片空白，正是这个缺陷当初
长期没被发现的原因。

### 数据导出（F9 已完成）

「设置」页（Ctrl+7）里一键导出：**每题一个 `.md`、图片一起带走、
双击用 Obsidian 打开即用、不需要装任何插件**。

- 正文走的是与 App 内部**同一个**序列化器，所以导出不可能因格式漂移而失真
- 用户状态以 `my_` 前缀并入 frontmatter（只读快照；有测试断言导出**不**修改事实源文件）
- 附 `题库索引.md`（按考点分组 + wiki 链接）与 `如何打开.md`

> 这一项的验收判据是"打开能用"，不是"有产出文件"。验证时靠**肉眼核对
> 导出样本**抓到一条断言抓不到的 bug：索引页 wiki 链接文字里的 `$` 会被
> Obsidian 当数学定界符，把链接吃掉一半。已修。

### 公式渲染（T3 已定）

底层是 **`katex`**（KaTeX 的 Flutter 封装）。关键点是**架构分层**：
`katex_dart` 是纯 Dart 引擎（只能给箱树或 SVG 字符串），Flutter 入口在另一个包
`katex` —— 它用 `KatexBoxPainter extends CustomPainter` 直接画 Canvas，
**不产 SVG**。所以那条"每条公式 460 KiB（99.8% 是内嵌字体）"的 SVG 路子在
App 里根本不存在，`flutter_svg` 也不需要。

| 实测项 | 数字 |
|---|---|
| 解析（`renderToBox`，实用路径） | **0.245 ms/式** |
| 解析（`renderToSvg`，不用） | 0.92 ms/式 + **460 KiB/式** |
| 1609 条真实语料解析失败 | 18 条（**真实知识点公式只占 2 条**） |

> ⚠️ **中文是这套渲染器的头号坑**：`\text{中文}` 被排进 `KaTeX_Main-Regular`，
> 而该字体**没有汉字字形**，也没配 `fontFamilyFallback`。实测
> **691 / 1523（45%）** 条真实公式至少有一个无字形字符（463 个汉字码点、
> 4453 次，外加全角标点 `（）、，：；`）。照 README 直接接入，会得到一个
> "中文全是方框、但所有测试都过"的渲染器。
>
> 处理方式是 `core/math/latex_text_split.dart`：把能**安全替换**的
> `\text{中文}` 摘成普通文本交给 Flutter 排（摘分率 **96%+**，有 1400 条真实
> 语料的测试守住下限）。摘不掉的 24 条（在 `\left..\right` 内、在
> `\begin..\end` 内、作为命令参数或上下标）原样交给 katex —— 最坏情况是偏窄，
> 不会破坏公式结构。
>
> 另外：`flutter test` **无法验证**字体回退 —— 默认 Ahem 测试字体把每个字形
> 都画成实心方框，对照组和被测组在 golden 图里长得一样。所以这里没有赌回退。

### 知识资产覆盖

| 科目 | 可考查单元 | 叶子节点 | 状态 |
|---|---|---|---|
| 数学一 | 19 | 141 | 🟢 覆盖完整（T19 清理后，原 198） |
| 数学二 | 11 | 58 | 🟢 完成（原 60） |
| 数学三 | 42（考频层是「节」） | 71 | 🟢 完成 |

> 这三行数字可以用一条命令复现：`python tools/data/knowledge_tree.py`
> （它会同时做结构校验：`level` 与 id 段数一致、父子完整、章节数 > 0）。

> **T19 已解决**：本体曾有约 25% 近义/重复叶子，同概念出现两个选项时
> LLM 只能掷硬币。现已合并到 **270 个叶子**（删除 59 个；内容按并集并入
> 保留项，零丢失）。
>
> 合并映射是**数据文件** `data/knowledge_points/merge_map.json`（每组都写了
> 为什么留这个删那个）；执行前必须跑 `python tools/data/verify_merges.py`
> —— 它会校验映射自洽性、算出内容搬运量、找出评测集 primary 冲突，
> 并检查**跑一遍管线会不会把删掉的节点搬回来**。

### 代码规模

```
app/lib     80 个文件   27,223 行   （不含 drift 生成产物）
app/test    48 个文件   12,659 行   （739 个用例，全部通过）
```

---

## 路线图：为什么现在只有数学

仓库名是 `exam-kb-agent`，而这一节要回答"扩到英语/政治/专业课要做什么"。
把整个仓库量过一遍之后的结论是：**骨架本来就与科目无关，真正绑死"数学"的只有 4 处。**

**与科目无关的部分**（一行都不用改）：双层存储（Markdown + SQLite）、增量索引与
FTS5 中文检索、FSRS 复习调度、错题本/复习/画像/组卷/导出、批量导入管线、
LLM 客户端与错误分类、平台能力抽象。

**绑死科目的 4 处**：

| # | 位置 | 现状 | 扩科目要做的 |
|---|---|---|---|
| 1 | **本体资产（那把尺子）** | `data/knowledge_points/math{1,2,3}.json`：270 个叶子，每个带定义、2–5 条真 LaTeX 公式、3–5 条常见陷阱、考频年份 | 每个科目一份。英语＝词汇/语法/题型树；政治＝马原/毛中特/史纲/思修/时政。**这是唯一的大工作量** |
| 2 | `Subject` 枚举 | `knowledge_repository.dart` 里硬编码三个值 | 改成数据驱动的科目注册表（id / 标签 / 本体文件 / 提示词模板 / 渲染方式） |
| 3 | 提示词 | `ingest_prompt.dart`「中文**考研数学**试题」、`tag_prompt.dart`「**考研数学**命题与阅卷专家」 | 按科目给模板：政治要认多选与材料题，英语要认篇章与设问 |
| 4 | 渲染与题型 | KaTeX 公式 + 公式键盘 + 数学题型枚举 | 英语/政治不需要公式渲染，需要长文本/材料题；题型枚举按科目走 |

**分阶段（按"能拿出手"排序）**：

| 阶段 | 内容 | 量级 |
|---|---|---|
| P1 | 文案与产品名对齐（本次已做） | 1 小时 |
| P2 | 科目注册表：`Subject` 枚举 → `data/subjects.json`，4 处参数化 | 0.5–1 天 |
| P3 | 第二个科目的**本体资产 + 考频 + 组卷模板 + 评测集** | 数周（数学本体就是这么来的） |
| P4 | 渲染器与题型按科目分派；非公式科目的录入体验 | 3–5 天 |

> **态度说明**：P2 很便宜，P3 很贵 —— 所以本项目不承诺"很快支持所有科目"。
> 想参与的话，**最有价值的贡献是本体资产**（某个专业课的知识点树 + 考频），
> 而不是重构代码。

### 召回率是怎么测的（重要）

四个金标准集角色分离，**引用数字时用最后一个**：

| 文件 | 题数 | 角色 | 实测 |
|---|---|---|---|
| `data/eval/gold_set.json` | 15 | 开发集（调参时一直看它） | 100% |
| `data/eval/gold_set_holdout.json` | 25 | 留出集（已用它诊断过，**污染**） | 96% |
| `data/eval/gold_set_final.json` | 15 | 第一次独立验证 | 100% |
| **`data/eval/gold_set_verify.json`** | 12 | **对外报数** | **91.7%** |

在同一个集合上反复调参会把它变成训练集。新增验证集时**不要回头改别名再跑** ——
那样它立刻退化成开发集。

---

## 快速开始

> **刚克隆仓库？** 只需一条命令 —— 双击 `run_app.bat`。
> 它会自动补上被忽略的 `app/assets/data/`（由 `data/` 生成）、设好国内镜像与代理、
> 必要时构建，然后启动 App。
>
> ```powershell
> git clone https://github.com/lysisiyi/exam-kb-agent.git
> cd exam-kb-agent
> .\run_app.bat
> ```

### 1. Flutter（已装好）

`D:\software\flutter`（3.47.4 stable），国内镜像 `FLUTTER_STORAGE_BASE_URL` /
`PUB_HOSTED_URL` 已配，`windows/` 脚手架与 sqlite3 原生库均已就绪。

> ⚠️ **唯一剩下的环境阻塞**：`flutter build windows` / `flutter run -d windows`
> 需要 **Windows 开发者模式**（插件走符号链接）。
> `flutter test` / `flutter analyze` **不需要**。
>
> ```powershell
> start ms-settings:developers
> ```
>
> 详见 **[`docs/SETUP.md`](docs/SETUP.md)** §5。

### 2. 同步知识资产

Flutter 不允许 `pubspec.yaml` 引用应用目录之外的路径，所以 `data/` 需要同步到
`app/assets/data/`：

```powershell
python tools/data/sync_assets.py
```

> ⚠️ **改完 `data/` 后必须重新同步**，否则 App 读到的还是旧数据。
> 改过**知识点名称**还要先重跑 `gen_aliases.py`（见"常用命令"）。

### 3. 跑起来

```powershell
cd app
flutter pub get
flutter test          # 739 个用例
flutter analyze       # 0 error / 0 warning
flutter run -d windows   # 需先开开发者模式
```

---

## 目录结构

```
├── app/                              Flutter 工程
│   ├── lib/
│   │   ├── core/
│   │   │   ├── layout/               断点系统（4 档，替代"按设备判断"）
│   │   │   ├── math/                 MathRenderer 抽象 + 渲染缓存
│   │   │   ├── platform/             平台能力抽象（OCR/图片/通知/安全存储）
│   │   │   ├── theme/                设计 token（与 UI 原型一致）
│   │   │   └── widgets/              自适应导航外壳
│   │   ├── data/
│   │   │   ├── markdown/             题目 Markdown 宽容解析器
│   │   │   ├── db/                   Drift schema（8 表 + FTS5 + 触发器）
│   │   │   └── index/                增量索引 + 中文全文检索
│   │   ├── domain/
│   │   │   ├── fsrs/                 FSRS-6 调度器（纯 Dart）
│   │   │   ├── fingerprint.dart      题目去重指纹
│   │   │   └── knowledge/            知识点模型（含 aliases）
│   │   ├── services/
│   │   │   ├── llm/                  服务商注册表 + 客户端 + 稳健 JSON + BYOK 配置
│   │   │   ├── tagger/               召回 + Prompt + 判定 + 评测器 + 缓存/用量台账
│   │   │   ├── review/               FSRS 落地：卡片对账 + 到期队列 + 打分
│   │   │   └── library/              录入闭环：草稿 → 落盘 → 刷索引
│   │   └── features/
│   │       ├── knowledge/            知识库三级树
│   │       ├── problems/             错题本列表 + 详情 + 删除
│   │       ├── review/               今日复习会话（揭晓式 + 三档打分）
│   │       └── entry/                录入页 + 公式键盘 + 考点选择器
│   ├── test/                         单元测试（739 个用例）
│   └── tool/
│       ├── recall_lab.dart           召回实验台（快速看每题失败细节）
│       └── tag_eval.dart             ★ T17：真实 LLM 的 Top-1 准确率评测
│
├── data/                             ★ 知识资产（单一事实源）
│   ├── knowledge_points/             知识点本体（math1/2/3，270 叶子）
│   │   ├── alias_overrides.json      人工维护的符号别名（T15）
│   │   └── merge_map.json            知识点合并映射（T19，49 组带理由）
│   ├── error_causes.json             错因受控词表（6 类 + 处方）
│   ├── exam_frequency.json           考频数据（三科 72 单元 / 282 热点）
│   ├── exam_templates.json           组卷模板
│   └── eval/                         四个金标准集（角色见上文）
│
├── tools/
│   ├── data/
│   │   ├── sync_assets.py            资产同步到 app/assets/
│   │   ├── merge_shards.py           多分片合并 + 覆盖缺口报告
│   │   ├── merge_knowledge.py        派生 exam_weight
│   │   ├── merge_frequency.py        考频分片合并 + 重新归一化
│   │   ├── dedupe_knowledge.py       按映射合并（数据驱动）+ 引用改挂
│   │   ├── verify_merges.py          ★ 合并执行前验证（含管线复活风险检查）
│   │   ├── refresh_suppressed.py     重算永久删除标记
│   │   ├── gen_aliases.py            生成召回别名（幂等）
│   │   ├── kp_probe.py               知识点条目速查
│   │   └── knowledge_tree.py         从树结构推导章节清单
│   ├── lint_dart.py                  Dart 静态自检（无 Flutter 时替代 analyze）
│   └── setup_windows.py              环境自举
│
├── docs/
│   ├── V1_PLAN.md                    ★ 开发计划（范围/里程碑/纪律/风险/验收）
│   ├── DATA_FORMAT.md                ★ 题目 Markdown 格式契约
│   ├── SETUP.md                      环境搭建
│   └── PROGRESS.md                   进度台账
│
└── ui-mockups/
    ├── index.html                    手机版 5 屏高保真原型
    └── ipad.html                     iPad 版 5 屏 + 适配规则
```

---

## 版本控制

仓库只提交**事实源与手写内容**，所有可再生的派生物都在 `.gitignore` 里：

| 被忽略 | 原因 | 怎么恢复 |
|---|---|---|
| `app/build/`、`app/.dart_tool/` | Flutter 构建产物 | `flutter pub get && flutter test` |
| `app/windows/flutter/ephemeral/` | Flutter 生成的临时目录（含 272 MB 的 `.pdb`） | `flutter build windows` |
| `app/assets/data/` | `data/` 的消费副本 | `python tools/data/sync_assets.py` |
| `tools/**/__pycache__/` | Python 字节码 | 自动 |

**刻意提交**：`tools/vendor/sqlite3.x64.windows.dll`（1.7 MB）。
它是 vendored 的预编译原生库 —— 国内网络拿不到 GitHub Releases 上的原件，
提交它才能让仓库自足（见 `docs/SETUP.md` §3 坑 3）。

`.gitattributes` 把源码行尾统一为 **LF**。这不是洁癖：
规划里要做 macOS / iOS 版，行尾不一致会让每次提交都出现"整个文件都改了"的假差异。

> 工作目录里另有两个**不属于本项目**的目录已被排除：
> `星匣AiGameJam/`（291 MB，另一个独立 npm 项目）与 `.perf/`（22 MB，DSH 性能测试脚手架）。
> 前者若需要版本控制，应在它自己的目录里 `git init`。

---

## 核心技术决策

### 1. 纯本地架构，零服务器

最重的计算（LLM 推理、公式识别）走**云端 API + 用户自己的 Key**：

- 服务器成本 **¥0**，运维成本 **¥0**
- 用户数据 100% 留在本机（隐私卖点）
- 无备案、无合规负担
- **AI 挂了产品不能挂** —— 错题本 / 复习 / 组卷 / 导出必须 100% 离线可用

### 2. 双层存储：Markdown 存内容，SQLite 存状态

```
题目内容  →  Markdown 文件（人类可读、可 Git、可迁移到 Obsidian）
用户状态  →  SQLite（高频写、需事务）
查询加速  →  SQLite FTS5 索引（可从 Markdown 全量重建）
```

> **铁律：绝不把用户状态写进 Markdown。**
> 一旦 `wrong_count` / `fsrs_state` 落进 `.md`，会同时陷入「高频重写文件」和「索引无法重建」两个坑。

### 3. `exam_weight` 统一算法

早期知识点文件与考频文件用了两套公式，已统一：

```
章节级（权威 = data/exam_frequency.json）
    raw     = total_appearances × avg_score
    chapter = round(raw / max(raw), 2)

叶子级（tools/data/merge_knowledge.py 派生）
    leaf = clamp(chapter × hotspot_boost × year_factor, 0.10, 1.00)
```

### 4. 章节解析沿 `parent_id` 向上找，不按段数硬切

| 科目 | 层级深度 |
|---|---|
| 数学一 | `math1 → calc → limit → 叶子`（4 段） |
| 数学三 | `math3 → calc → limit → 节 → 叶子`（5 段） |

且**必须限定同一学科前缀** —— 否则 `math3.prob.limit`（大数定律）会错配到
`math3.calc.limit`（极限）。

### 5. 知识点带 `aliases`（召回别名）

召回层是纯规则的，靠字面重合。而知识点名往往是复合短语、题干只写符号：

```
知识点名：正态分布及其标准化计算
题干：    设 $X\sim N(0,1)$，求 $P\{|X|<1\}$
```

两边没有任何共同子串 → 规则层完全匹配不上 → 正确答案进不了候选集 → LLM 再强也没用。
`aliases` 就是把知识点名"翻译"成题干可能的样子：

| 来源 | 例子 | 生成方式 |
|---|---|---|
| 名称片段 | 「单调性、极值与最值」→ 极值 / 最值 | `gen_aliases.py` 自动派生 |
| 符号写法 | `X\sim N(\mu,\sigma^2)` / `\iint_D` | `alias_overrides.json` 人工维护 |

含反斜杠的走公式 token 匹配，其余走文本包含匹配。
**实测把召回率从 73.3% 提到 91.7%**（见上文评测表）。

### 6. 平台能力走抽象接口

`OcrService` / `ImageSourceService` / `NotifyService` / `SecureStore` 四个接口，
V1 只实现 Mock + Windows。加 Android/iOS/macOS 时只补实现，不改调用方。

### 7. UI 按宽度断点，不按设备判断

```dart
LayoutBreakpoint bp(double w) {
  if (w < 600)  return compact;   // 手机竖屏
  if (w < 900)  return medium;    // 手机横屏
  if (w < 1200) return expanded;  // iPad 竖屏 / Split View
  return large;                    // PC / iPad 横屏
}
```

**绝不写 `if (Platform.isAndroid)` 来决定布局** —— Windows 窗口拖窄、iPad 分屏时会崩。

---

## 常用命令

```powershell
# 同步知识资产（改完 data/ 必跑）
python tools/data/sync_assets.py

# 检查资产是否已同步（CI 用）
python tools/data/sync_assets.py --check

# 合并多分片知识点 + 报告覆盖缺口
python tools/data/merge_shards.py --dry-run

# 派生 exam_weight
python tools/data/merge_knowledge.py --subject math1 --check

# 生成召回别名（幂等；改过知识点名之后必跑）
python tools/data/gen_aliases.py --all
python tools/data/gen_aliases.py --all --check    # 自检：应为 0 处待更新

# 找冗余知识点（只提示，不删除）
python tools/data/dedupe_knowledge.py --suggest
python tools/data/dedupe_knowledge.py --review    # 打印候选对的完整内容

# 知识点合并（T19）：**改映射后先验证，再执行**
python tools/data/verify_merges.py                # 自洽性/内容搬运量/评测集主考点冲突/管线复活风险
python tools/data/dedupe_knowledge.py --all       # 改挂引用 + 按映射合并（不可逆）
python tools/data/refresh_suppressed.py           # 重算"永久删除标记"

# 查某个知识点的名称/定义/公式/陷阱（补别名时用）
python tools/data/kp_probe.py math1.prob.rv1.normal_distribution

# Dart 静态自检（无 Flutter 时替代 flutter analyze）
python tools/lint_dart.py

# 测试与运行
cd app
flutter test
flutter run -d windows

# 召回率快速实验台（比 flutter test 快，能看每题失败细节）
cd app
dart run tool/recall_lab.dart --misses --candidates 5
dart run tool/recall_lab.dart --set gold_set_verify --rank

# T17：真实 LLM 的 Top-1 准确率评测（需要自己的 API Key）
dart run tool/tag_eval.dart --key=sk-xxxx --set=gold_set --out=t17_gold.json
dart run tool/tag_eval.dart --key=sk-xxxx --set=gold_set_verify --limit=3   # 先小样本试错
```

---

## 参考的开源项目

| 项目 | 借鉴点 |
|---|---|
| [ruoshui03/kaoyan-PinPaper](https://github.com/ruoshui03/kaoyan-PinPaper) | 题库 Markdown 结构、三版式 PDF、考频软偏好组卷 |
| [shuangzhebai/gaokao-analyzer](https://github.com/shuangzhebai/gaokao-analyzer) | OR-Tools 组卷、IRT 掌握度模型、FTS5 检索 |
| [chnjames/exam-forge](https://github.com/chnjames/exam-forge) | **三层验证**（沙箱 + SymPy + LLM-as-judge） |
| [open-spaced-repetition/py-fsrs](https://github.com/open-spaced-repetition/py-fsrs) | FSRS 调度算法（本项目移植为纯 Dart） |
| [opendatalab/MinerU](https://github.com/opendatalab/MinerU) | 中文学术 PDF 解析（公式→LaTeX、表格→HTML） |
| [opendatalab/UniMERNet](https://github.com/opendatalab/UniMERNet) | 数学公式识别（CDM 追平 Mathpix） |
| KnowTS（[arXiv 2406.13885](https://doi.org/10.48550/arxiv.2406.13885)） | LLM 做数学题知识点打标的方法论 baseline |

---

## 许可与声明

- **知识资产中的考频数据是估算值**，非官方逐题统计。详见
  `data/exam_frequency.json` 的 `data_confidence` 字段。UI 上展示时须标注「估算值」。
- 题库版权：本项目不内置任何受版权保护的题库。用户自行导入的资料需自行确认合规性。
