...
class ProblemService {
  ...
  /// 删除一道题：**用户状态 → 复习历史 → 知识点关联 → 索引行 → Markdown 文件**。
  ///
  /// ## ⚠️ 这个顺序是约定，不是数据库强制的
  ///
  /// 曾有注释称"`problem_knowledge.problem_id` 是外键，所以必须先删关联行" ——
  /// **那个外键并不存在**（`tables.dart` 里没有 `references()` 声明）。
  /// 所以下面这个顺序取消任何一步都不会被 SQLite 拦下来，只会静默产生
  /// 悬空行。改这个函数时要自己保证顺序。
  ///
  /// 顺带说明为什么**不**改成真外键：给 `problem_knowledge` 加
  /// `references(problemsIndex.id, onDelete: cascade)` 之后，
  /// 索引重建（`IndexBuilder._removeById` 会先删索引行再重插）会级联删掉
  /// 关联行——这没问题；但同一条链路上 `problems_index` 与
  /// `user_problem_state` 之间若也加外键，重建索引就会**连用户复习进度一起删**。
  /// 与其引入这种隐蔽的破坏，不如把派生表当派生表用：
  /// 关联行可以在任何一次索引重建里全部重算，悬空行不需要靠约束兜底。
  ///
  /// ## 为什么 Markdown 必须最后删
  ///
  /// 它是内容的事实源。中途失败时宁可留下"有文件没索引"的孤儿
  /// （重建索引即可恢复），也不要出现"有索引没文件"（列表点进去读不到内容）。
  ///
