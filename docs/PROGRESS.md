...
| T41 | **Windows release 版 GUI 应用没有 stdout，异常退出不一定留事件日志** | "应用闪一下就没了"类问题只能靠猜 | 已缓解：启动日志（`<题库>/library/startup.log`）+ 按需阶段探针（`DSH_STARTUP_PROBE=1`）。仍未解决的是**UI 运行期**的崩溃 —— 那需要 `FlutterError.onError` 上报到同一个日志，等真遇到再加 |
| T42 | `run_app.bat` 里的代理端口 `26561` 是硬编码的 | 用户换了加速器端口就得改脚本 | 已缓解：只在端口**确实在监听**时才用它；脚本注释里写了怎么读系统代理设置 |
| **T43** | `pdf` 包内置字体**不含中文字形** | 导出的 PDF 里中文可能显示异常或缺失 | V1 **如实告知**：导出结果带 `caveats`，UI 弹对话框说明并给出替代方案（导出 Markdown 包）。未解决的是"PDF 里正常显示中文" —— 两条出路都不合适：`PdfGoogleFonts` 要联网（与纯本地/国内可用冲突）、自带中文字体 5–10 MB（为导出 PDF 给包体加这么多不划算）。V2 可考虑按需下载字体，或改用 HTML→PDF |
