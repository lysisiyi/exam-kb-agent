...
| M6 组卷 + 导出 | 🟡 **F9 数据导出已完成**；组卷引擎与 PDF 未开始 | ~30% |
| M7 PC 批量导入 | ⚪ 未开始 | 0% |
| M8 画像 + 打磨 | ⚪ 未开始 | 0% |
...
**测试用例合计：435 个**（`flutter test` 全绿；`flutter analyze` **No issues found**）
（tagger 96 · 录入闭环域/服务 35 · 数据层 35 · katex 渲染+摘分 32 · 平台服务 33 ·
Markdown/指纹 24 · 录入界面 21 · 复习仓库+会话 20 · 缓存与台账 14 · 数据导出 13 ·
录入页 12 · 别名 11 · 每日提醒 10 · 渲染缓存 10 · 错题本列表 8 · 外壳导航 7 ·
真实目录入口 6 · 设置页 5 · 窗口 5 · 召回率评测 5 · 主题 4 · 其他）
