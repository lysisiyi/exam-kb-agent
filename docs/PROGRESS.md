...
| ~~N10~~ | ~~M7 PC 批量导入（文件夹 → 解析 → 查重 → 入库）~~ | ✅ 主流程已完成 | — |
| **N11** | M8 学情画像（薄弱章节 / 错因分布 / 掌握度曲线） | 复习日志已开始积累 | 3 天 |
| **N12** | 补 Windows 平台服务实跑验证（DPAPI / file_selector / 通知） | 需要用户真机操作 | 0.5 天 |
| **N13** | M8 打磨：空态/错误态统一、性能复核、连续自用 7 天 | N11 | 2 天 |
| **N14** | 用**真实 API + 真实扫描件**验收 M7（≥100 题、成功率 ≥85%） | 需要用户真机操作与 Key | 0.5 天 |

**M0–M7 主流程已通，下一个里程碑是 M8 画像 + 打磨。**

N14 单独列出来，是因为 M7 的目标是"一次导入 ≥100 题、成功率 ≥85%"，
而这个数字**只能在真机上量**：沙箱里没有网络、没有 Key、也没有用户的
扫描件。代码侧能测的（三协议编码、提炼解析、防编造、查重、取消、
失败隔离、预估）已经用假 HTTP 适配器全部覆盖（40 条），
但"模型到底看不看得懂手写公式"这个问题，测试永远回答不了。
