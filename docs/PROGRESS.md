...
| M8 画像 + 打磨 | 🟡 **打磨完成**（空态/错误态 + 性能实测与优化 + 健壮性）；**F7 掌握度画像未做** | ~55% |
...
| **T50** | **公式不跟随系统字号缩放**，而摘出来的中文跟随 | 系统字号调到 150–200% 时，同一条公式里数学是 1x、中文是 2x | 已确认机制：中文走 `TextSpan`（`Text.rich` 会按 `MediaQuery.textScaler` 缩放），数学走 `WidgetSpan` 里的 `katex.Math`（内部 `TextPainter` 不带 scaler）。修法是给 `katex` 传的字号乘上 scaler，但**不能连 TextSpan 一起乘**（会双重缩放）。没做是因为**视觉结果在 widget 测试里验证不了**（Ahem 字体），改动面又不小 —— 等真机上确认它确实难看再动 |
...
| **T52** | `secure_store.deleteAll` 在 `readAll()` 抛异常时**什么都不删却也不报错** | 用户点"清空配置"可能没有任何效果，而界面会说成功 | 目前没有生产调用方（清空配置走的是逐个 `delete`）。真要用它之前必须先让它能把失败传出去 |
