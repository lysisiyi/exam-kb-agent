...
这是 60 秒目标的关键。逼用户在录入现场翻 198 个知识点，
...
| 别名的两种来源 | 例子 | 覆盖 |
|---|---|---|
| **名称片段**（自动派生） | 「单调性、极值与最值」→ 极值 / 最值 | 144/198 个叶子 |
...
| 产出 | 路径 | 规模 | 状态 |
|---|---|---|---|
| **知识点本体 · 数学一** | `data/knowledge_points/math1.json` | **226 叶子 / 19 章** | 🟢 权重 226/226 |
...
**知识点本体总计：460 个节点 / 357 个叶子节点，考频权重派生 357/357**
...
| T4 | ~~数学二/三考频数据缺失~~ | ✅ **已解决** | 三科考频齐备，357/357 权重派生成功 |
| T5 | `exam_frequency.json` 自述 `data_confidence = medium-low` | 权重仅可作相对参考 | 生产前用真实真题逐题标注替换；**UI 上必须标注「估算值」** |
| T6 | 数学一 19 章粒度不均（curvesurface 15 vs system 9） | 候选集过大可能降低标注准确率 | **等 M3 测出 Top-1 准确率再决定**；不达标则收敛到 130–150 |
...
| T18 | 知识点本体曾被多次 AI 生成污染（28 个冗余） | 已修复（226→198 叶子） | 已建白名单去重工具；后续新增知识点需走同一流程 |
| T19 | **本体仍有约 25% 近义/重复叶子** | 直接拉低 LLM Top-1（见下） | **优先级最高**，但需先定本体粒度策略 |
| T20 | 去重工具的自动检测有天花板 | 只能发现部分重复 | 需一次性人工通读全部叶子名 |
...
#### 下一步（建议）
一次性人工通读 198 个叶子名，产出一份 (保留 → 删除) 映射，再执行。
