...
| M8 画像 + 打磨 | 🟢 **打磨完成**（含 T40/T41/T50/T52 等技术债）；**F7 掌握度画像未做** | ~60% |
...
| **N11** | **F7 掌握度画像**（薄弱章节 / 错因分布 / 掌握度曲线）—— M8 的另一半，也是 V1 必做十项里唯一没有界面的 | 复习日志已开始积累 | 3 天 |
...
| **N16** | 连续自用 7 天（V1 验收表最后一条） | N11 | 7 天 |

**M0–M7 主流程已通；M8 的打磨已完成，只差 F7 掌握度画像。**
